...
namespace LY.VirtualMeter.Core
{
    ...
    {// <summary>
        ...
        /// <returns></returns>
        [DllImport(@"EncrypGW\DLL_SERVER_698\WinSocketServerVirtuaMeter.dll")]
        ...
        public static int iObj_Meter_Formal_GenReadData(int iKeyState, int cOperateMode, string cTESAMNO, string cRandHost, string cReadData,ref string cOutData,ref string cOutMAC)
        {
            lock (obj)
            {
                try
                {
                    LogInfoHelper.WriteLog("异常日志", "正在计算MAC");
                    var t = IEncryptionFunction698.iObj_Meter_Formal_GenReadData(iKeyState, cOperateMode, cTESAMNO, cRandHost, cReadData, ref cOutData, ref cOutMAC);
                    //var t = Obj_Meter_Formal_GenReadData(iKeyState, cOperateMode, cTESAMNO, cRandHost, cReadData, cOutData, cOutMAC);
                    LogInfoHelper.WriteLog("异常日志", "计算MAC完成，返回值"+t);
                    if (t!=0)
                    {
                        var t2 = ConnectDevice(AppData.加密机IP, AppData.加密机端口, "5");
                        if (t2 != 0)
                        {
                            LogInfoHelper.WriteLog("异常日志", "加密机连接失败,错误码" + t2);
                        }
                        else   //连接成功了，在次调用
                        {
                            System.Threading.Thread.Sleep(100);//给一点缓冲时间
                            //t = Obj_Meter_Formal_GenReadData(iKeyState, cOperateMode, cTESAMNO, cRandHost, cReadData, cOutData, cOutMAC);
                            t = IEncryptionFunction698.iObj_Meter_Formal_GenReadData(iKeyState, cOperateMode, cTESAMNO, cRandHost, cReadData, ref cOutData, ref cOutMAC);
                        }
                    }
                    return t;
