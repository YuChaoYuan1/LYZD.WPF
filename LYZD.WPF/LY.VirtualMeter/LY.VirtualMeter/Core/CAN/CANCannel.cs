...
namespace LY.VirtualMeter.Core.CAN
{
    public class CANCannel : PortBase
    {
        ...
        public CANCannel(string strCanParams, uint devType, uint devIndex, uint canId)
        {

            DeviceType = devType;//设备类型
            DeviceIndex = devIndex;//设备索引号（卡号）
            CanId = canId;//Can 通道号
            CanParams = strCanParams;//CAN通讯参数
        ...
        public override void SendData(byte[] Data, UInt32 ID)
        {
            ...
            lock (_Lock)
            {
                ...
                for (int i = 0; i < Frame_Num; i++)
                {
                    ...
                    sendobj.ID = ID;
                    //Thread.Sleep(5);
                    //System.IO.File.AppendAllText("Log\\虚拟表日志\\下行" + GetBw(ID) + ".txt", DateTime.Now.ToString("HH:mm:ss fff ") + "发:" + GetByteToStr(sendobj.Data, sendobj.DataLen) + Environment.NewLine);
                    ...
                    if (res > 0)
                    {
                        ...
                        for (UInt32 i = 0; i < res; i++)
                        {
                            ...
                            if (obj.RemoteFlag == 0)
                            {
                                ...
                                //System.IO.File.AppendAllText("Log\\虚拟表日志\\下行" + GetBw(obj.ID) + ".txt", DateTime.Now.ToString("HH:mm:ss fff ") + "收:" + GetByteToStr(listbyteTmp.ToArray(), listbyteTmp.ToArray().Length) + Environment.NewLine);
