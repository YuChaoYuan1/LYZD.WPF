...
namespace LYZD.Verify.ControlTest
{
    ...
    /// 营业报停功控
    /// </summary>
    public class BusinessStop : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        public override void Verify()
        {
            try
            {
                ...
                SetData_698("0701278105030002045023011C07E10808FFFFFF1C08330909FFFFFF14000000000000138800", "添加营业报停控配置单元总加组1");
                ...
                WaitTime("延时等待", 65);
                SetData_698("0701308105060050230100", "控制投入_营业报停控_总加组1");
                        ...
                        for (int i = 0; i < MeterNumber; i++)
                        {
                            if (meterInfo[i].YaoJianYn & !bol_Qualified[i])
                            {
                                ...
                                if (TalkResult[i] == 0)
                                {
                                    ...
                                    if (GetData(RecData, i, 12, EnumTerimalDataType.e_string).Substring(2,1) == "1")
                                    {
                                        TempData[i].Tips = "告警状态：" + GetData(RecData, i, 12, EnumTerimalDataType.e_string);
                                        TempData[i].Resoult = "合格";
                                    }
                                    else
                                    {
                                        TempData[i].Resoult = "不合格";
                                        //ResultDictionary["召测终端总加组1当前控制状态"][i] = "告警状态：" + GetData(RecData, i, 12, EnumTerimalDataType.e_string) + "|00100000";
                                        TempData[i].Tips = "营业报停控_未告警！";
                                    }
                                    if (TempData[i].Resoult == "合格")
                ...
                SetData_698("0701358105070050230100", "控制解除_营业报停控_总加组1");
                WaitTime("延时等待", 60);
                MessageAdd("召测终端总加组1当前控制状态", EnumLogType.流程信息, true);
                for (int i = 0; i < MeterNumber; i++)
                {
