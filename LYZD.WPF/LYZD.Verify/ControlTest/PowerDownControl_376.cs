...
namespace LYZD.Verify.ControlTest
{
    ...
    /// 当前功率下浮控
    /// </summary>
    public class PowerDownControl_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        public override void Verify()
        {
            try
            {
                ...
                MessageAdd("设置终端功控功率下浮参数...",EnumLogType.流程信息);
                ...
                MessageAdd("设置终端功率下浮功控定值浮动系数...",EnumLogType.流程信息);
                ...
                MessageAdd("设置终端功率下浮功控定值...",EnumLogType.流程信息);
                ...
                MessageAdd("设置终端功率下浮厂休功控参数...",EnumLogType.流程信息);
                ...
                MessageAdd("设置终端功率下浮功控解除...",EnumLogType.流程信息);
                SetData = UsefulMethods.ConvertStringToBytes("");
                ...
                MessageAdd("功率下浮控投入...",EnumLogType.流程信息);
                SetData = UsefulMethods.ConvertStringToBytes("01B0050603030303");
                TalkResult = TerminalProtocalAdapter.Instance.WriteData(5, 1, 12, SetData, RecData, MaxWaitSeconds_Write);
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    ...
                    {
                    TempData[i].StdData = "功率下浮控告警";
                        if (TalkResult[i] == 0)
                        {
                            ResultDictionary[""][i] = (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(4, 1) == "1" ? "功率下浮控告警" : "功率下浮控未告警") ;
                            if (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(4, 1) != "1")
                            {
                ...
                MessageAdd("功率下浮功控解除...",EnumLogType.流程信息);
                SetData = UsefulMethods.ConvertStringToBytes("");
                TalkResult = TerminalProtocalAdapter.Instance.WriteData(5, 1, 20, SetData, RecData, MaxWaitSeconds_Write);
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                    TempData[i].StdData = "功率下浮控未告警";
                        if (TalkResult[i] == 0)
                        {
                            TempData[i].Data = (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(4, 1) == "1" ? "功率下浮控告警" : "功率下浮控未告警");
                            if (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(4, 1) != "0")
                            {
