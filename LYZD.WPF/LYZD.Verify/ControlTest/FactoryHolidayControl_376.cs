...
namespace LYZD.Verify.ControlTest
{
    ...
    /// 厂休功控
    /// </summary>
    public class FactoryHolidayControl_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        public override void Verify()
        {
            try
            {
                ...
                MessageAdd("厂休控投入...",EnumLogType.流程信息);
                SetData = UsefulMethods.ConvertStringToBytes("");
                TalkResult = TerminalProtocalAdapter.Instance.WriteData(5, 1, 10, SetData, RecData, MaxWaitSeconds_Write);
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    ...
                    {
                        TempData[i].StdData = "厂休控告警";
                        if (TalkResult[i] == 0)
                        {
                            TempData[i].Data = (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(6, 1) == "1" ? "厂休控告警" : "厂休控未告警");
                            if (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(6, 1) != "1")
                            {
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                        ...
                        if (TalkResult[i] == 0)
                        {
                            TempData[i].Data = (GetData(RecData, i, 8, EnumTerimalDataType.e_bs8).Substring(6, 2) == "11" ? "各轮都跳闸11" : "有轮次未跳闸");
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                        TempData[i].StdData = "厂休控未告警";
                        if (TalkResult[i] == 0)
                        {
                            TempData[i].Data = (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(6, 1) == "1" ? "厂休控告警" : "厂休控未告警");
                            if (GetData(RecData, i, 11, EnumTerimalDataType.e_bs8).Substring(6, 1) != "0")
                            {
                                TempData[i].Resoult = "不合格";
                                TempData[i].Tips = "不正确";
                            }
