...
namespace LYZD.Verify.AccurateMeasurementTest
{
    ...
    ///无功功率基本误差
    /// </summary>
    public class BaseReactivePowerError_376 : VerifyBase
    {
        float Ia = 0f;
        float Pa = 0f;
        float Pb = 0f;
                ...
                for (int i = 0; i < MeterNumber; i++)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        ...
                        else
                        {
                            ...
                            float fTerminalErr = (float.Parse(GetData(RecData, i, 8, EnumTerimalDataType.e_float))*1000 - StdPQ) / fStandard * 100;
                            if (Math.Abs(fTerminalErr) > ErrorLimit)
                                Resoult[i] = "不合格";

                            ResultDictionary["标准功率"][i] = StdPQ.ToString("F2");
                            ResultDictionary["终端功率"][i] = (float.Parse(GetData(RecData, i, 8, EnumTerimalDataType.e_float)) *1000).ToString("F2");
