...
namespace LYZD.Verify.AccurateMeasurementTest
{
    ...
    /// 谐波影响
    /// </summary>
    public class BaseHarmonicError : VerifyBase
    {
        float ErrorLimit = 1;
        /// <summary>
        /// 检定参数是否合法
        ...
        public override void Verify()
        {
            ...
            try
            {
                ...
                float Ub = OnMeterInfo.MD_UB;

                float[] temHarmonicContent = new float[60];
                float[] temHarmonicPhase = new float[60];
                
                    temHarmonicContent[5 - 2] = 10.0f;
                    temHarmonicPhase[5 - 2] = 90.0f;
                
                MessageAdd("正在下发谐波参数", EnumLogType.流程信息, true);
                WaitTime("正在下发谐波参数",10);
                //EquipmentData.DeviceManager.ZH3001SetPowerGetHarmonic("1", "1", "1", "1", "1", "1", temHarmonicContent, temHarmonicPhase, true);
                MessageAdd("开始设置谐波", EnumLogType.流程信息, true);
                WaitTime("开始设置谐波", 10);

                if (!PowerOn(OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, 1.0f, 1.0f, 1.0f, 0, 240, 120, 0f, 240f, 120f))
                {
                ...
                MessageAdd("正在关闭谐波", EnumLogType.流程信息, true);
                WaitTime("正在下发谐波关闭参数", 5);
                EquipmentData.DeviceManager.ZH3001SetPowerGetHarmonic("1", "1", "1", "1", "1", "1", temHarmonicContent, temHarmonicPhase, false);
                
                WaitTime("开始关闭谐波", 10);

                if (!PowerOn(OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, 0, 0, 0, 0, 240, 120, 0f, 0f, 0f))
                {
                    MessageAdd("升源失败,退出检定", EnumLogType.错误信息);
                    return;
                }
                MessageAdd("关闭谐波完成", EnumLogType.流程信息, true);
            }
            catch (Exception ex)
            {
