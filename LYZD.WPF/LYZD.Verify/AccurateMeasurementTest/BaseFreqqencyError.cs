...
namespace LYZD.Verify.AccurateMeasurementTest
{
    ...
    /// 频率影响实验
    /// </summary>
    public class BaseFreqqencyError : VerifyBase
    {
        float ErrorLimit = 1;
        /// <summary>
        /// 检定参数是否合法
        ...
        public override void Verify()
        {
            ...
            try
            {
                ...
                MessageAdd("调整频率升源开始", EnumLogType.流程信息, true);
                ...
                MessageAdd("开始恢复频率", EnumLogType.流程信息, true);
                if (!PowerOn(OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, OnMeterInfo.MD_UB, Ia, Ia, Ia, Cus_PowerYuanJian.H, PowerWay.正向有功, "1.0", 50))
                {
                    MessageAdd("升源失败,退出检定", EnumLogType.错误信息);
                    return;
                }
