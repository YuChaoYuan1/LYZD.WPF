        ...
        public override void Verify()
        {
            ...
            for (int i = 0; i < meterInfo.Length; i++)
            {
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    float tmpLmt = 0.25F;
                   tmpLmt= GetErrBianChaLmt(meterInfo[i]);

                    if (Clfs==WireMode.单相)
                    {
                        if (VarietyErr2 <= tmpLmt && VarietyErr3 <= tmpLmt)
                        {
                    ...
                    {
                        if (VarietyErr1 <= tmpLmt && VarietyErr2 <= tmpLmt && VarietyErr3 <= tmpLmt)
                        {
                            ResultDictionary["结论"][i] = Const.合格;
                        }
                        else
