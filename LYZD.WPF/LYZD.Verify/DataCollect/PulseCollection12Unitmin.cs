...
namespace LYZD.Verify.DataCollect
{
    ...
    /// 12个/分脉冲量采集
    /// </summary>
    public class PulseCollection12Unitmin : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        public override void Verify()
        {
            try
            {
                ...
                //deviceDriver.StartPuleslOutput(bol_VerifyFlags, 2, 1, 216, 0.2f, 1.6f);
                SetPulseOutput(GetYaoJian(), 0x03, 0.2f, 0.5f, 216, 0.2f, 0.5f, 216);
                Thread.Sleep(500);
                    ...
                    if (meterInfo[i].YaoJianYn)
                    {
                       TempData[i].StdData = "720";
                        if (TalkResult[i] == 0)
                        {
                            ...
                            TempData[i].Data = double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 + "," + string.Format("{0:0.00}", (Math.Abs((double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 720) / 7.2))) ;

                            if (Math.Abs((double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 720) / 7.2) <= 2)
                            {
                                //ResultDictionary["有功功率"][i] = double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 + "," + string.Format("{0:0.00}", (Math.Abs((double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 720) / 7.2))) + "|720";
                            }
                            else
                            {
                               TempData[i].Resoult="不合格";
                    ...
                    if (meterInfo[i].YaoJianYn)
                    {
                       TempData[i].StdData = "216";
                        if (TalkResult[i] == 0)
                        {
                            ...
                            TempData[i].Data = double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 + "," + string.Format("{0:0.00}", (double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 216) / 2.16) ;

                            if (Math.Abs((double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 216) / 2.16) <= 2)
                            {
                                //ResultDictionary["当日正向有功电量"][i] = double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 + "," + string.Format("{0:0.00}", (double.Parse(GetData(RecData, i, 7, EnumTerimalDataType.e_double)) / 10 - 216) / 2.16) + "|216";
                            }
                            else
                            {
                               TempData[i].Resoult="不合格";
