...
namespace LYZD.Verify.DataCollect
{
    ...
    /// 电能表实时数据
    /// </summary>
    public class EnergyRealData : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        protected override bool CheckPara()
        {
            ...
            ResultNames = new string[] { "正向有功电能示值", "反向有功电能示值", "组合无功1电能示值", "组合无功2电能示值", "正向有功最大需量", "电压", "电流", "电能表时钟", "事件类数据", "透明转发", "结论" };
            return true;
        }
        ...
        public override void Verify()
        {
            ...
            for (int kk = 0; kk < 1; kk++)
            {
                ...
                AddItemsResoult("正向有功电能示值", TempData);
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                ...
                AddItemsResoult("反向有功电能示值", TempData);
                ...
                AddItemsResoult("组合无功1电能示值", TempData);
                ...
                AddItemsResoult("组合无功2电能示值", TempData);
                ...
                AddItemsResoult("正向有功最大需量", TempData);
                ...
                MessageAdd("事件类数据", EnumLogType.流程信息, true);
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                        Talkers[i].Framer698.sAPDU = " 09 01 06 00 64 01 07 05 00 00 00 00 00 02 00 00 04 30 15 07 00 30 17 07 00 30 03 0A 00 20 14 02 00 00".Replace(" ", "");
                        setData[i] = Talkers[i].Framer698.ReadData(Talkers[i].Framer698.sAPDU);//代理读取电能量类数据
                    }
                }
                TalkResult = TerminalProtocalAdapter698.Instance.WriteData(setData, RecData, MaxWaitSeconds_Read485);
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                        TempData[i].StdData = "1008";
                        if (TalkResult[i] == 0)
                        {
                            string sTmp = GetData(RecData, i, 9, EnumTerimalDataType.e_string);
                            if (sTmp != "1008")
                                TempData[i].Resoult = "不合格";
                            TempData[i].Data = sTmp;
                        }
                ...
                AddItemsResoult("事件类数据", TempData);
