...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电流反向事件
    /// </summary>
    public class TerminalCurrentReversed_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电流反向事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电流反向事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00010000000000000000000000000000");
                ...
                DateTime dtHappen = DateTime.Now.AddMinutes(-1);//记录当前时间，事件发生的时间一定比这个晚
                 ...
                 MessageAdd("设置终端参数 F25.测量点基本参数",EnumLogType.错误信息);
                ...
                //TerminalProtocalAdapter.Instance.WriteData_Afn04(4, 1, 25, "57.7,1.5,0.25", RecData, MaxWaitSeconds_Write);

                ResetTerimal(2);

                PowerOn(Xub, Xub, Xub, xIb, xIb, xIb,0,240,120,0,240,120);
                ...
                PowerOn(Xub, Xub, Xub, xIb, xIb, xIb, 0, 240, 120, 0, 240, 300);
                ...
                BaseVerifyUnit_EventData(EventName, "ERC9", 4, dtHappen);


                PowerOn(Xub, Xub, Xub, 0, 0, 0, 0, 240, 120, 0, 240, 120);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9, EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "1" && GetData(RecData, i, 11, EnumTerimalDataType.e_string) == "发生" && GetData(RecData, i, 12, EnumTerimalDataType.e_string) == "C相反向")
                            {
