...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表超差事件
    /// </summary>
    public class MeterErr_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "电能表超差事件", "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表超差事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000008000000000000000000000000");
                ...
                SetData = UsefulMethods.ConvertStringToBytes("2080012C01");
                ...
                ControlVirtualMeter("Cmd,DLS,8000.66");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC28", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7,EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9,EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10,EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 11,EnumTerimalDataType.e_string) == "发生" && GetData(RecData, i, 14,EnumTerimalDataType.e_string) == "2")
                                {
