...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表运行状态字变位事件
    /// </summary>
    public class RunStatusChange_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表运行状态字变位事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表运行状态字变位事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                TalkResult = TerminalProtocalAdapter.Instance.WriteData_Afn04(4, 0, 9, "33", RecData, MaxWaitSeconds_Write);
                ...
                ControlVirtualMeter("Cmd,SZT,0002,0000,0000,0000,0000,0000,0000");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC33", 4, dtHappen);
            }
            catch (Exception ex)
            {
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if ((GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9, EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 11, EnumTerimalDataType.e_string) == "0200"))
                            {
