...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表示度下降事件
    /// </summary>
    public class MeterDropIn_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表示度下降事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表示度下降事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000004000000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,DLS,500,400,100,100,100,100");
                WaitTime("事件发生抄表", VerifyConfig.WaitTime_CopyMeter);

                //ControlVirtualMeter("Cmd,DLS,5000,4000,1000,1000,1000,1000");
                //WaitTime("事件恢复抄表", VerifyConfig.WaitTime_CopyMeter);
                ...
                BaseVerifyUnit_EventData(EventName, "ERC27", 4, dtHappen);
                ControlVirtualMeter("Cmd,Set,220,220,220,5,5,5,0,0,0,1,0");
            }
            catch (Exception ex)
            {
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7,EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9,EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10,EnumTerimalDataType.e_string) == "2")
                                {
