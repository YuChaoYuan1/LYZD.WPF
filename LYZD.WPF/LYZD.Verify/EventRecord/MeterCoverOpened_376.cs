...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表开表盖事件
    /// </summary>
    public class MeterCoverOpened_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表开表盖事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表开表盖事件";
                MessageAdd("设置" + EventName + "有效", EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000000100000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,03300D00,0");
                ControlVirtualMeter("Cmd,03300D01,EE");
                ResetTerimal(2);
                WaitTime("测试前抄表", VerifyConfig.WaitTime_CopyMeter);
                ControlVirtualMeter("Cmd,03300D00,1");
                ControlVirtualMeter("Cmd,03300D01,000709110313000010110313000010000000000000000000000000000000000000000000000020000000000000000000000000000000000000000000");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC37", 4, dtHappen);
            }
            catch (Exception ex)
            {
