...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电压越限事件
    /// </summary>
    public class VoltageOverflow_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电压越限事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电压越限事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00008000000000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,Set,300,220,220,5,5,5,45,45,45,1,0");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC24", 4, dtHappen);
            }
            catch (Exception ex)
            {
