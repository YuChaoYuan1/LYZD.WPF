...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 磁场异常事件
    /// </summary>
    public class MagneticAnomalyEvent_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "磁场异常事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "磁场异常事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000000800000000000000000000000");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC40", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "2")
                            {
