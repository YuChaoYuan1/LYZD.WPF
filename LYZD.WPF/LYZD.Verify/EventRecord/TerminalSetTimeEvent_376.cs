...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 终端对时事件
    /// </summary>
    public class TerminalSetTimeEvent_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "对时事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "对时事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000000000100000000000000000000");
                ...
                WaitTime("等待抄表", VerifyConfig.WaitTime_CopyMeter);
                ...
                BaseVerifyUnit_EventData(EventName, "ERC41", 4, dtHappen);
