...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表断相次数变更事件
    /// </summary>
    public class MeterCutOfPhase_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表断相次数变更事件", "结论" };
            return true;
        }
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if ((GetData(RecData, i, 7,EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9,EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10,EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 12,EnumTerimalDataType.e_string).Contains("电能表断相次数变化")))
                                {
