...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表飞走事件
    /// </summary>
    public class MeterFly : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "电能表飞走事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                SetData_698("060124310D0900030100", "设置事件有效标志");
                ...
                ControlVirtualMeter("Cmd,DLS,5000.68,4000,1000,1000,1000,1000");

                //dttmp = dttmp.AddMinutes(15);
                //SetTime_698(dttmp, 0);
                WaitTime("等待事件发生,", 300);

                MessageAdd("读取上1次电能量飞走事件",EnumLogType.流程信息);
                for (int i = 0; i < MeterNumber; i++)
                {
                    if (meterInfo[i].YaoJianYn)
                    {
                        ...
                        ret = TerminalProtocol.Encryption.IEncryptionFunction698.Obj_Formal_GetRandHost(i, ref  Talkers[i].Framer698.cOutRand);
                         Talkers[i].Framer698.sAPDU = "1000" + GetMiWenLen("05 03 2B 31 0D 02 00 09 01 04 00 20 22 02 00 00 20 1E 02 00 00 20 20 02 00 00 20 24 02 00 00    ".Replace(" ", "")) + "05 03 2B 31 0D 02 00 09 01 04 00 20 22 02 00 00 20 1E 02 00 00 20 20 02 00 00 20 24 02 00 00 ".Replace(" ", "") + "0110" +  Talkers[i].Framer698.cOutRand;
                ...
                AddItemsResoult("电能表飞走事件", TempData);
