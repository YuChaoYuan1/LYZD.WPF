...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表飞走事件
    /// </summary>
      public class MeterFly_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "电能表飞走事件", "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表飞走事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000010000000000000000000000000");
                ...
                //ControlVirtualMeter("Cmd,Set,220,220,220,5,5,5,0,0,0,1,10");
                //ControlVirtualMeter("DLS00100");
                //ResetTerimal(2);
                //WaitTime("测试前抄表", VerifyConfig.WaitTime_CopyMeter);
                //ControlVirtualMeter("DLS00500");
                //WaitTime("事件发生抄表", VerifyConfig.WaitTime_CopyMeter);


                ControlVirtualMeter("Cmd,DLS,8100");
                WaitTime("事件发生抄表", VerifyConfig.WaitTime_CopyMeter + 10);
                ...
                BaseVerifyUnit_EventData(EventName, "ERC29", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7,EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9,EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10,EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 14,EnumTerimalDataType.e_string) == "4")
                            {
