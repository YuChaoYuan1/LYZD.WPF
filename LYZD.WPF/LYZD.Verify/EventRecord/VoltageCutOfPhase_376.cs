...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电压断相事件
    /// </summary>
    public class VoltageCutOfPhase_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电压断相事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电压断相事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00020000000000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,Set,40,220,220,0,5,5,45,45,45,1,0");
                ControlVirtualMeter("Cmd,SZT,0000,0000,0000,0080,0000,0000,0000");
                ControlVirtualMeter("DXS");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC10", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7,EnumTerimalDataType.e_string) == EventNum && GetData(RecData, i, 10,EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 11,EnumTerimalDataType.e_string) == "发生" && GetData(RecData, i, 12,EnumTerimalDataType.e_string).Contains("断相"))
                                {
