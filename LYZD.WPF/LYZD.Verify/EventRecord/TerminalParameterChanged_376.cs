...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 终端参数变更事件
    /// </summary>
     public class TerminalParameterChanged_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "终端参数变更事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "终端参数变更事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("04000000000000000000000000000000");
                ...
                WaitTime("等待", 5);
                 MessageAdd("设置终端参数 F4.主站电话号码和短信中心号码",EnumLogType.错误信息);
                RecData.Clear();  //清除上一次数据
                SetData = UsefulMethods.ConvertStringToBytes("32532086532086533253208653208653");
                TalkResult = TerminalProtocalAdapter.Instance.WriteData(4, 0, 4, SetData, RecData, MaxWaitSeconds_Write);

                WaitTime("等待", 5);
                 MessageAdd("设置终端参数 F4.主站电话号码和短信中心号码",EnumLogType.错误信息);
                RecData.Clear();  //清除上一次数据
                SetData = UsefulMethods.ConvertStringToBytes("33532086532086533253208653208653");
                TalkResult = TerminalProtocalAdapter.Instance.WriteData(4, 0, 4, SetData, RecData, MaxWaitSeconds_Write);
                ...
                BaseVerifyUnit_EventData(EventName, "ERC3", 4, dtHappen);
            }
            catch (Exception ex)
            {
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if ((GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9, EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 11, EnumTerimalDataType.e_string) == "00000800"))
                            {
