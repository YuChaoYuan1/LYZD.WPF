...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 视在功率越限事件
    /// </summary>
    public class ViewPowerOverFlow_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "视在功率越限事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "视在功率越限事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000002000000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,Set,220,220,220,5,5,5,0,0,0,1,1");
                WaitTime("测试前抄表",VerifyConfig.WaitTime_CopyMeter);
                ControlVirtualMeter("Cmd,Set,236.3636,220,220,5.5,5,5,0,0,0,1,1");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC26", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if (GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9, EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 11, EnumTerimalDataType.e_string) == "发生" && GetData(RecData, i, 12, EnumTerimalDataType.e_string) == "视在功率越上上限")
                            {
