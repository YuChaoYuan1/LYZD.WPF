...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表电池欠压事件
    /// </summary>
    public class MeterUndervoltage_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表电池欠压事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表电池欠压事件";
                MessageAdd("设置" + EventName + "有效", EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00100000000000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,SZT,0000,0000,0000,0000,0000,0000,0000");
                WaitTime("测试前抄表", VerifyConfig.WaitTime_CopyMeter);
                ControlVirtualMeter("Cmd,SZT,000C,0000,0000,0000,0000,0000,0000");
                ...
                TalkResult = TerminalProtocalAdapter.Instance.ReadEvent(14, 3, 2, RecData, NormalEventCount, MaxWaitSeconds_Write);

                BaseVerifyUnit_EventData(EventName, "ERC13", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if ((GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetDateTime(GetData(RecData, i, 9, EnumTerimalDataType.e_datetime)) > dtHappen && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 12, EnumTerimalDataType.e_string).Contains("电能表电池欠压")))
                            {
