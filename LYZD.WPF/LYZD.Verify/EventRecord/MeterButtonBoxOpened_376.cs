...
namespace LYZD.Verify.EventRecord
{
    ...
    /// 电能表开端钮盒事件
    /// </summary>
    public class MeterButtonBoxOpened_376 : VerifyBase
    {
        /// <summary>
        /// 检定参数是否合法
        /// </summary>
        ...
        {
            ResultNames = new string[] { "禁止终端主动上报", "终端事件记录配置设置", "终端事件计数当前值", "电能表开端钮盒事件", "结论" };
            return true;
        }
        public override void Verify()
        {
            try
            {
                ...
                string EventName = "电能表开端钮盒事件";
                 MessageAdd("设置" + EventName + "有效",EnumLogType.错误信息);
                SetData = UsefulMethods.ConvertStringToBytes("00000000200000000000000000000000");
                ...
                ControlVirtualMeter("Cmd,03300E00,0");
                ControlVirtualMeter("Cmd,03300E01,EE");
                ResetTerimal(2);
                WaitTime("测试前抄表", VerifyConfig.WaitTime_CopyMeter);
                ControlVirtualMeter("Cmd,03300E00,1");
                ControlVirtualMeter("Cmd,03300E01,000509110313000010110313000010000000000000000000000000000000000000000000000020000000000000000000000000000000000000000000");
                ...
                BaseVerifyUnit_EventData(EventName, "ERC38", 4, dtHappen);
                ...
                if (meterInfo[i].YaoJianYn)
                {
                    ...
                    if (TalkResult[i] == 0)
                    {
                        if (RecData[i].Length > 9)
                        {
                            ...
                            if ((GetData(RecData, i, 7, EnumTerimalDataType.e_string) == EventNum && GetData(RecData, i, 10, EnumTerimalDataType.e_string) == "2" && GetData(RecData, i, 11, EnumTerimalDataType.e_string) == "1"))
                            {
