...
{
    public class ReadMacCheck : VerifyBase
    {
        public override void Verify()
        {
            ...
            MessageAdd("设置模拟表不回应采集数据", EnumLogType.提示与流程信息, true);
            WaitTime("延时，", 120);
            ...
            for (int i = 0; i < MeterNumber; i++)
            {
                if (meterInfo[i].YaoJianYn)
                {
                    if (TalkResult[i] == 0)
                    {
                        TempData[i].Data = GetData(RecData, i, 10, EnumTerimalDataType.e_string) + "_成功数量:" + GetData(RecData, i, 13, EnumTerimalDataType.e_string);
                    }
                    else
                    {
                        TempData[i].Tips = "无回复";
            ...
            AddItemsResoult("读取采集任务监控数据_抄表不回复", TempData);
            ...
            AddItemsResoult("读取采集任务监控数据_验证", TempData);
