    ...
    internal class COM32 : IConnection
    {
        ...
        public bool SendData(ref byte[] vData, bool IsReturn, int WaiteTime, int MaxByte)
        {
            lock (ThreadObject)
            {
                ...
                //验证数据完整性
                if (!DataValidation698(BitConverter.ToString(vData)))//报文不完整-开始延时等待
                {
                    TmpTime1 = DateTime.Now;
                    while (TimeSub(DateTime.Now, TmpTime1) < 5000)   //5秒超时
                    {
        ...
        public bool SendData(ref byte[] vData, bool IsReturn, int WaiteTime, int MaxByte, int MaxWaitSeconds)
        {
            lock (ThreadObject)
            {
                ...
                spCom.DiscardInBuffer();
                //Console.WriteLine("┏SendData:{0}", BitConverter.ToString(vData));
               //Console.WriteLine("发送=========:{0}", BitConverter.ToString(vData));
