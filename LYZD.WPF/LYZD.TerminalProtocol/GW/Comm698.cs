...
namespace LYZD.TerminalProtocol.GW
{
    ...
    public class Comm698
    {
        ...
        /// <param name="iv"></param>
        /// <returns></returns>
        public static string getdjmd(string siv)
        {
            int iv = Convert.ToInt16(siv, 16);
            switch (iv)
            {
                case 0:
                    return "时间间隔:不冻结";
        ...
        public static string time_s(string strData)
        {
            ...
            else
            {
                try
                {
                    hour = Convert.ToInt16(strData.Substring(0, 2), 16);
                    minute = Convert.ToInt16(strData.Substring(2, 2), 16);
                    second = Convert.ToInt16(strData.Substring(4, 2), 16);

                    strDateStr = hour.ToString("00") + ":" + minute.ToString("00") + ":" + second.ToString("00");
                    return strDateStr;
        ...
        public static string gd1(string Tstr)
        {
            ...
            {
                string Ts = "";
                if (Tstr.Substring(0, 4) == "EEEE") return "2099-1-1";
                ...
                Ts = (Convert.ToInt16(Tstr.Substring(10, 1)) * 10 + Convert.ToInt16(Tstr.Substring(11, 1))).ToString("2000") + "-" + Ts;
        ...
        public static string gd2(string strData)
        {

            int fltSum;//求和
            int y;//次幂
            int intStrLen;//字符串长度
            string strVoid = "eeeeeeeeeeeeeeeeeeee"; string strDefValu = "-999999";
            strData = strData.Trim();
            intStrLen = strData.Length;
            //判断是不是无效帧数据"ee ee.."
            ...
            else
            {
                try
                {
                    ...
                    {
                        return (fltSum * Math.Pow(10, y)).ToString();
                    }
                    else
                    {
                        return (-fltSum * Math.Pow(10, y)).ToString();
        ...
        public static string gd15(string strData)
        {
            ...
            else
            {
                try
                {
                    ...
                    strDateStr = year.ToString("2000") + "-" + month.ToString("00") + "-" + day.ToString("00")
        ...
        public static int ExplanP(string p)
        {
            ...
            if (tint == 0) return 0;
            tint2 = ShiLiuToShi(p.Substring(2, 2));
            return (tint2 - 1) * 8 + tint;
        }
        ...
        public static string HourTd_h(string strData)
        {
            return (Comm.GetAnyValue(Convert.ToInt16(strData.Substring(0, 2), 16), 4, 5) * 10 + Comm.GetAnyValue(Convert.ToInt16(strData.Substring(0, 2), 16), 0, 3)).ToString();
        }
        ...
        /// <returns></returns>
        public static string ExplanBinFun(string strData)
        {
            try
            {
                double tempBin = 0;
                ...
                {
                    tempBin += Convert.ToInt32(strls.Substring(i * 2, 2), 16) * Math.Pow(256, strls.Length / 2 - 1 - i);
                }
                if (strls.Length == 1)
                {

                    tempBin += Convert.ToInt32(strls.Substring(0, 1), 16);
                }
                return tempBin.ToString();
            }
            catch
            {
                return "9999";
        ...
        {
            string str1 = ""; string strBu2 = "";
            for (int j = strData.Length / 2; j > 0; j--)
            {
                str1 = Convert.ToString(Convert.ToInt16(strData.Substring((j - 1) * 2, 2), 16), 2).PadLeft(8, '0');
                for (int i = 0; i < 8; i++)
                {
                    strBu2 += str1.Substring(i, 1);
                }
            }
            return strBu2;
        }
        ...
        /// <param name="strData"></param>
        /// <returns></returns>
        public static string GetEventAl(string strData)
        {
            string str1 = ""; string strBu2 = "";
            strData = revStr(strData);
            for (int i = 0; i < strData.Length / 2; i++)
            {
                str1 += Convert.ToString(Convert.ToInt16(strData.Substring(i * 2, 2), 16), 2).PadLeft(8, '0');
            }
        ...
        public static string Reduce33H(string Pstr)
        {
            try
            {
                ...
                for (Ti = 1; Ti <= Tlen; Ti = Ti + 2)
                    Tstr = Tstr + string.Format("{0:X2}", (Convert.ToInt16(Pstr.Substring(Ti - 1, 2), 16) + 0x33) % 0x100);

                string str = Tstr.Replace(" ", ""); string strBu2 = "";
                if (str.Length > 0)
                {
                    if (str.Length % 2 == 0)
                    {
                        for (int i = 0; i < Convert.ToInt32(str.Length) / 2; i++)
                            strBu2 += str.Substring(str.Length - 2 - i * 2, 2);
                    }
                }
        ...
        /// <param name="strData"></param>
        /// <returns></returns>
        public static int ExplanByteWeiValue(string strData)
        {
        ...
        /// <param name="strData"></param>
        /// <returns></returns>
        public static string ExplanNo(string strData)
        {
            return strData;
        }

        public static string RExplanNo(string strData)
        {
            return revStr(strData);
        }
    }
}
