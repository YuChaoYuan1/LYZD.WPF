...
namespace LYZD.DAL.DataBaseView
{
    /// <summary>
    /// 表显示帮助类
    /// </summary>
    public class ResultViewHelper
    {
        ...
        public static TableDisplayModel GetTableDisplayModel(string viewID, bool reloadFlag = false)
        {
            ...
            DynamicModel viewModel = DALManager.ApplicationDbDal.GetByID(EnumAppDbTable.T_VIEW_CONFIG.ToString(), string.Format("AVR_VIEW_ID='{0}'", viewID));
            if (viewModel != null)
            {
                ...
                if (viewModel.GetProperty("AVR_COL_NAME") != null && viewModel.GetProperty("AVR_COL_SHOW_NAME") != null)
                {
                    ...
                    if (pkDisplayString.Trim() != string.Empty)
                    {
                        ...
                        for (int i = 0; i < fields.Length; i++)
                        {
                            ...
                            //if (fields[i].Trim() == "MD_ITEM_VALUE")//不需要分项结论显示在主界面
                            //    continue;
