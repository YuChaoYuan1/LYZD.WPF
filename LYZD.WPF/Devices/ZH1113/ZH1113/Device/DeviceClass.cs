...
namespace ZH
{
    ...
    internal class ZH1113_PulseOutputPacket : ZH3001SendPacket
    {
        ...
        public void SetPara(byte conType,byte bwNum, int fq1,byte pWM1, int pulseNum1 , int fq2, byte pWM2, int pulseNum2)
        {
            BwNum = bwNum;
            Fq1 = fq1;
            Fq2 = fq2;
        ...
        protected override byte[] GetBody()
        {
            ...
            buf.PutInt(Fq1);
            buf.Put(PWM1);
            buf.PutInt(PulseNum1);
            buf.PutInt(Fq2);
            buf.Put(PWM2);
            buf.PutInt(PulseNum2);
            buf.Put(ConType);
            ...
            else
            {
                ...
                else
                {
                    if (data[0] == 0x8C)
                    {
                        ...
                        for (int i = 0; i < OutTriggerMode.Length; i++)
                        {
