...
namespace ZH.SocketModule
{
    ...
    public class SockPool
    {
        ...
        public void AddUdpSock(string szSockName, IPAddress remoteIp, int remotePort, int localPort, int BasePort, string HaveProtocol, int MaxWaitMSecond, int WaitSecondPerByte, Cus_EmDeviceType m_EmDeviceType)
        {
            lock (objLock)
            {
                ...
                Connection s = new Connection(remoteIp, remotePort, localPort, BasePort, HaveProtocol, MaxWaitMSecond, WaitSecondPerByte, m_EmDeviceType);
        ...
        /// 注册Can 端口号
        /// </summary>
        /// <param name="szName">注册名称</param>
        /// <param name="strParams">设备通讯参数 如 500Kbps</param>
        /// <param name="deviceType">设备 类型</param>
        /// <param name="deviceInd">设备索引号</param>
        /// <param name="canInd">CAN 通道号 总线编号</param>
        /// <param name="CanCmtID">设备对应ID号</param>
        /// <param name="strSetting">设备波特率</param>
        /// <param name="maxWaittime">最大等待时间(ms)</param>
        /// <param name="waitSencondsPerByte">字节间间隔（ms)</param>
        public void AddCanSock(string szName,StCanParams sc,
            uint CanCmtID, string strSetting, int maxWaittime, int waitSencondsPerByte)
        {
            lock (objLock)
            {
                if (DicSock.ContainsKey(szName))
                {
                    return;
                }
                Connection s = new Connection(sc, CanCmtID, strSetting, maxWaittime, waitSencondsPerByte);
                DicSock.Add(szName, s);
                dicLock.Add(szName, new object());
            }
        }
