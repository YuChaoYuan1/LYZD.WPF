...
namespace ZH.SocketModule.Sock
{
    ...
    internal class Connection
    {
        ...
        public Connection(IPAddress remoteIp, int remotePort, int localPort, int intBasePort, string HaveProtocol, int MaxWaitMSecond, int WaitSecondsPerByte, Cus_EmDeviceType m_EmDeviceType)
        {
            connection = new UDPClient(remoteIp.ToString().Split(':')[0], localPort, remotePort, intBasePort, HaveProtocol, m_EmDeviceType) as IConnection;
        ...
        public Connection(StCanParams sc, uint CanCmtID, string strSetting, int maxWaittime, int waitSencondsPerByte)
        {
            connection = new CanCommuinication(sc, CanCmtID);

            connection.MaxWaitSeconds = maxWaittime;
            connection.WaitSecondsPerByte = waitSencondsPerByte;
            //打开
            bool bValue = connection.Open();
        }
        ...
        public bool Send(Packet.SendPacket sendPack, Packet.RecvPacket recvPack, string PortName)
        {
            ...
            //fjk
            ZH485.LogModel.LogFrameInfo Lf = new ZH485.LogModel.LogFrameInfo();
            ...
            if (!Send(ref vData, sendPack.IsNeedReturn, sendPack.WaiteTime(), sendPack.MaxByte(), 3000))
            {
            ...
            if (sendPack.IsNeedReturn == true && recvPack != null)
            {
                //Console.WriteLine("收到数据{0},{1}", BitConverter.ToString(vData, 0), PortName);
                if (false)
                {
                    //2018带协议 ，解包
                }
